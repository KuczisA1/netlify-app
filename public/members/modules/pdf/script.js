...
(function(){
  ...
  // ===== SIMPLE MESSAGE / EMBED HELPERS =====
  function ensureEmbedElements(){
    let iframe=document.getElementById('embedFrame');
    if(!iframe){ iframe=document.createElement('iframe'); iframe.id='embedFrame'; iframe.className='embedFrame'; iframe.allow='fullscreen'; document.body.appendChild(iframe); }
    let msg=document.getElementById('embedMsg');
    if(!msg){ msg=document.createElement('div'); msg.id='embedMsg'; msg.className='embedMsg'; msg.hidden=true; document.body.appendChild(msg); }
    return { iframe, msg };
  }
  function showEmbedMessage(text, hint){ const {msg}=ensureEmbedElements(); msg.innerHTML='<div><b>'+(text||'Błąd')+'</b>'+(hint?('<div class="hint">'+hint+'</div>'):'')+'</div>'; msg.hidden=false; }
  function hideEmbedMessage(){ const msg=document.getElementById('embedMsg'); if(msg) msg.hidden=true; }
  function showEmbedFrame(src){ const {iframe}=ensureEmbedElements(); iframe.src=src; iframe.style.display='block'; }
  ...
  async function tryLoadByAliasId(){
    ...
    if(!/^[A-Za-z0-9_:-]{1,50}$/.test(alias)){ showEmbedMessage('Nieprawidłowy identyfikator.', 'Dozwolone: litery, cyfry, _, -, :'); return true; }
    try{
      const url='/.netlify/functions/pdf-key?key='+encodeURIComponent(alias);
      ...
      return true;
    }catch(e){ showEmbedMessage('Błąd: '+(e&&e.message||'Nie udało się pobrać ID pliku.'), 'Sprawdź alias w URL oraz konfigurację funkcji.'); return true; }
  }
...
${body}
  ...
  (async ()=>{
    ...
    const okAlias = await tryLoadByAliasId();
    const ok = okAlias || await tryLoadFromParams();
